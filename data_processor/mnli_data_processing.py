...
def build_mnli_dataset(output_dir="data_mnli", max_train=10000, max_val=2000, max_test=3000, seed=42):
    output_dir = os.path.abspath(output_dir)
    raw = load_dataset("glue", "mnli")

    train_raw = raw["train"].shuffle(seed=seed)
    val_raw = raw.get("validation_matched") or raw.get("validation")
    ...
    print(f"Saved MNLI dataset to {output_dir}")
    for split in ds:
